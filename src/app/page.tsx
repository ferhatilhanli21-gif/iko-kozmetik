    ...
    <section ref={ref} className="relative min-h-[100svh] flex items-center overflow-hidden grid-bg">
      ...
      <motion.div style={{ opacity }} className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 w-full pt-20 pb-16 md:pt-28 md:pb-24">
        <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}
          ...
          className="font-heading text-[1.85rem] sm:text-5xl md:text-7xl font-normal leading-[1.15] mb-5 max-w-4xl overflow-visible">
          <span className="italic text-gradient inline-block pt-1">{t.hero.title1}</span>
          <br />
...
function FairGallery() {
  ...
  function handleTouchEnd() {
    ...
    if (isSwiping.current && Math.abs(diff) > 50) {
      ...
      // Tap — overlay'ı gizle, altındaki videoya tıkla
      overlayRef.current.style.pointerEvents = 'none'
      requestAnimationFrame(() => {
        const el = document.elementFromPoint(touchStartX.current, touchStartY.current)
